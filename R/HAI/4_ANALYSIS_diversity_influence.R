...
prepare_paired_data <- function(data) {
  ...
  analysis_df <- data.frame(
    # 問題的指標
    Q_TTR = questions$TTR,
    Q_Yule_K = questions$Yule_K,
    Q_Simpson_D = questions$Simpson_D,
    Q_Herdan_C = questions$Herdan_C,
    Q_Brunet_W = questions$Brunet_W,
    # Q_Honore_R = questions$Honore_R,
    
    # AI回答的指標
    AI_TTR = ai_responses$TTR,
    AI_Yule_K = ai_responses$Yule_K,
    AI_Simpson_D = ai_responses$Simpson_D,
    AI_Herdan_C = ai_responses$Herdan_C,
    AI_Brunet_W = ai_responses$Brunet_W,
    # AI_Honore_R = ai_responses$Honore_R,
    
    # 人類回答的指標
    H_TTR = human_responses$TTR,
    H_Yule_K = human_responses$Yule_K,
    H_Simpson_D = human_responses$Simpson_D,
    H_Herdan_C = human_responses$Herdan_C,
    H_Brunet_W = human_responses$Brunet_W
    # H_Honore_R = human_responses$Honore_R
  )
  
  return(analysis_df)
}

analysis_df <- prepare_paired_data(diversity_data)
...
# 對每個指標進行相關性分析
metrics <- c("TTR", "Yule_K", "Simpson_D", "Herdan_C", "Brunet_W")
...
# ===== 4. 結果輸出 =====
# 創建結果輸出目錄
dir.create("output/diversity_influence", recursive = TRUE, showWarnings = FALSE)

# 輸出相關性分析結果
sink("output/diversity_influence/correlation_analysis.txt")
cat("詞彙多樣性相關性分析結果\n")
cat("==========================\n\n")
...
# 輸出回歸分析結果
sink("output/diversity_influence/regression_analysis.txt")
cat("詞彙多樣性回歸分析結果\n")
cat("==========================\n\n")
...
for(metric in metrics) {
  ...
  ggsave(
    sprintf("output/diversity_influence/%s_scatter.png", metric),
    p,
    width = 10,
    height = 6
  )
...
print("分析完成！結果已保存到 output/diversity_influence/ 目錄") 